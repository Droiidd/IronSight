...
public class BanditTask extends BukkitRunnable {
  ...
  // IN SECONDS
  private final int mobRespawnTime = 60;
  private final int horseFullCallTime = 10;
  private final int wantedTownTimer = 10;
  private final boolean escapeFlag = false;
  private final boolean wildernessFlag;
  private final FrontierMob undeadMiner;
  private final FrontierMob berserkerMiner;
  private final FrontierMob corrodedMiner;
  private final FrontierMob raider;
  private final FrontierMob ranger;
  private final FrontierMob wolf;
  private final FrontierMob raiderBrute;
  private final FrontierMob rabbit;
  private final FrontierMob fox;
  private final FrontierMob boar;
  private final FrontierMob cow;
  private final List<FrontierMob> miners = new ArrayList<>();
  private final List<FrontierMob> animals = new ArrayList<>();
  private final List<FrontierMob> raiders = new ArrayList<>();
    ...
    if (tick % 3 == 0) {
          ...
          if (b.isTrackingNPC()) {
            p.spigot()
                .sendMessage(
                    ...
                    new TextComponent(
                        b.getTrackedNPC() + " " + distanceMsg + ChatColor.GRAY + " blocks away!"));
          } else {
            p.spigot()
                .sendMessage(
                    ChatMessageType.ACTION_BAR,
                    new TextComponent(
                        b.getTrackingFrontierLocation().getLocName()
                            + " "
                            + distanceMsg
                            + ChatColor.GRAY
                            + " blocks away!"));
            ...
            if (b.getTargetedPlayer().isOnline()) {
              ...
              p.spigot()
                  .sendMessage(
                      ChatMessageType.ACTION_BAR,
                      new TextComponent(
                          String.valueOf(distanceMsg) + ChatColor.GRAY + " blocks away!"));
            }
          }
        }
      }
      ...
      if (b.isRespawning()) {
        ...
        if (!p.getOpenInventory()
            .getTitle()
            .equalsIgnoreCase(ChatColor.DARK_GRAY + "Choose Town:")) {
      ...
      if (b.isJailed()) {
        if (!currentLoc.getType().equals(LocationType.PRISON)) {
          ...
          if (!b.isEscaping()) {
            ...
            p.sendTitle(
                ChatColor.RED + String.valueOf(ChatColor.BOLD) + "Escapee!",
                ChatColor.GRAY + "Return to jail or gain bounty");
          }
        } else {
          // They are in prison!
          b.setEscaping(false);
          ...
          p.spigot()
              .sendMessage(
                  ...
                  new TextComponent(
                      ChatColor.RED
                          + ""
                          + (wantedTownTimer - wantedTownCounter)
                          + ChatColor.RESET
                          + " seconds remaining."));
      ...
      if (currentLoc.getType().equals(LocationType.ILLEGAL)
          || currentLoc.getType().equals(LocationType.OIL_FIELD)) {
      ...
      if (b.isSummoningHorse()) {
        ...
        if (horseTimer == horseFullCallTime) {
          ...
          p.getLocation()
              .getWorld()
              .playSound(p.getLocation(), Sound.BLOCK_AZALEA_LEAVES_PLACE, 1, 0);
          ...
          p.sendMessage(
              ChatColor.AQUA
                  + b.getHorseBeingSummoned().getHorseName()
                  + ChatColor.GRAY
                  + " has arrived!");
      ...
      if (location.getPlayersInside().isEmpty()) {
        if (location.isMobsSpawned()) {
          ...
          for (Map.Entry<UUID, LivingEntity> npcEnt : npcEnts.entrySet()) {
            if (location
                .getLocName()
                .equalsIgnoreCase(npcs.get(npcEnt.getKey()).getFrontierLocation().getLocName())) {
        ...
        if (location.isProcsSpawned()) {
          ...
          for (Map.Entry<UUID, LivingEntity> procEnt : procEnts.entrySet()) {
            if (location
                ...
                .equalsIgnoreCase(
                    procs
                        .get(ChatColor.stripColor(procEnt.getValue().getCustomName()))
                        .getLocation()
                        .getLocName())) {
