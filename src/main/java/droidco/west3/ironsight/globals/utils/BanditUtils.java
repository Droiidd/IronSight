...
public class BanditUtils {
  ...
  public static void releasePrisoner(Player p, Bandit b) {
    ...
    p.sendTitle(
        ChatColor.GRAY + "You are released from" + ChatColor.RED + " Prison!", "Good luck...");
  ...
  public static void loadScoreBoard(
      Player p, Bandit b, int secCombatBlock, int minWanted, int secWanted) {
    ...
    String contractTitle =
        BanditUtils.getContractorTitle(b) == null ? "" : BanditUtils.getContractorTitle(b);
    // Do the same for teams

    // Get banking info
    String wallet =
        ChatColor.GREEN + "Wallet: " + ChatColor.RESET + b.getWallet() + ChatColor.GOLD + "g";
    ...
    String contractorLvl =
        ChatColor.AQUA
            + "Contractor "
            + ChatColor.GRAY
            + "["
            + BanditUtils.getContractorLvlColor(b)
            + b.getContractorLvl()
            + ChatColor.GRAY
            + "]";
    ...
    String secWantedStr =
        singleDigits.contains(secWanted) ? "0" + secWanted : String.valueOf(secWanted);
    String wanted =
        ChatColor.DARK_RED + "< Wanted > " + ChatColor.RESET + minWanted + ":" + secWantedStr;
    // Combat blocked timer
    String secCmbtStr =
        singleDigits.contains(secCombatBlock)
            ? "0" + secCombatBlock
            : String.valueOf(secCombatBlock);
    String combatBlock =
        ChatColor.DARK_PURPLE + "Combat Blocked " + ChatColor.RESET + "0:" + secCmbtStr;

    String seperator = "---------------";

    Objective objective =
        sb.registerNewObjective(
            ChatColor.DARK_RED + String.valueOf(ChatColor.BOLD) + "IronSight",
            ChatColor.GRAY + String.valueOf(ChatColor.BOLD) + "Iron Sight");
    ...
    if (b.getActiveContract() != null) {
      ...
      String contractListing =
          "\"" + b.getActiveContract().getListingName() + ChatColor.RESET + "\"";
  ...
  public static String getContractorTitle(Bandit b) {
    return switch (b.getContractorTitle()) {
      case 0 -> "";
      case 1 -> ChatColor.GRAY + "Miner";
      case 2 -> ChatColor.GRAY + "Cowboy";
      case 3 -> ChatColor.DARK_GREEN + "Tracker";
      case 4 -> ChatColor.GREEN + "Medic";
      case 5 -> ChatColor.DARK_RED + "Raider";
      case 6 -> ChatColor.YELLOW + "Explorer";
      default -> "";
    };
  }
  ...
  public static String getRandomTip() {
    ...
    tips.add(
        ChatColor.AQUA
            + "TIP: "
            + ChatColor.GRAY
            + "Don't forget to send your horse back to the stable when you arrive at your destination");
    tips.add(
        ChatColor.AQUA
            + "TIP: "
            + ChatColor.GRAY
            + "Do /c to open your contractor profile or view active contracts");
    tips.add(ChatColor.AQUA + "TIP: " + ChatColor.GRAY + "Contracts reset every hour!");
    tips.add(
        ChatColor.AQUA
            + "TIP: "
            + ChatColor.GRAY
            + "Track unknown locations or NPC's with the tracker item");
    tips.add(
        ChatColor.AQUA
            + "TIP: "
            + ChatColor.GRAY
            + "Every named river has a unique signature fish");
    tips.add(ChatColor.AQUA + "TIP: " + ChatColor.GRAY + "Start a contract to earn gold!");
    tips.add(
        ChatColor.AQUA + "TIP: " + ChatColor.GRAY + "Contracts are the quickest way to earn money");

    tips.add(
        ChatColor.AQUA
            + "TIP: "
            + ChatColor.GRAY
            + "Go to a contractor NPC to view available contracts");
    tips.add(
        ChatColor.AQUA + "TIP: " + ChatColor.GRAY + "Locate a contractor NPC to start a contract");
    tips.add(
        ChatColor.AQUA
            + "TIP: "
            + ChatColor.GRAY
            + "Earn more rewards by leveling up your contractor level");
    tips.add(ChatColor.AQUA + "TIP: " + ChatColor.GRAY + "Do /call or /horse to summon your steed");
    tips.add(
        ChatColor.AQUA
            + "TIP: "
            + ChatColor.GRAY
            + "Something look off or is too confusing? Report to Droiid!");
  ...
  public static Inventory newVaultUI(Player p) {
    ...
    Inventory newVaultUI =
        Bukkit.createInventory(
            p, 27, p.getDisplayName() + ": " + ChatColor.DARK_AQUA + "Open an Account");
  ...
  public static Inventory vaultUI(Player p) {
    Bandit b = Bandit.getPlayer(p);
    Inventory vaultUI =
        Bukkit.createInventory(p, 54, p.getDisplayName() + ": " + ChatColor.DARK_AQUA + "Vault");
  ...
  public static Inventory vaultUpgradeUI(Player p) {
    Bandit b = Bandit.getPlayer(p);
    Inventory vaultUpgradeUI =
        Bukkit.createInventory(
            p, 27, p.getDisplayName() + ": " + ChatColor.DARK_AQUA + "Upgrade Vault");
