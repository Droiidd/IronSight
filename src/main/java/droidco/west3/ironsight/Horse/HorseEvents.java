...
public class HorseEvents implements Listener {
    ...
    public void onHorseSummon(InventoryClickEvent e) {
        ...
        if (e.getView().getTitle().equalsIgnoreCase(ChatColor.AQUA + p.getDisplayName() + ChatColor.GRAY + "'s horses")) {
            e.setCancelled(true);
            Bandit b = Bandit.getPlayer(p);
            List<FrontierHorse> horses = b.getHorses();
            if (e.getCurrentItem().getType() == Material.NAME_TAG) {
                for (FrontierHorse horse : horses) {
                    if (e.getCurrentItem().getItemMeta().getDisplayName().equalsIgnoreCase(horse.getHorseName())) {
                        p.closeInventory();
