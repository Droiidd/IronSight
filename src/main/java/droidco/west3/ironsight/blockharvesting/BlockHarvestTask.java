    ...
    switch (blockType) {
      case FOLIAGE -> {
        block
            .getLocation()
            .getWorld()
            .playSound(block.getLocation(), Sound.ENTITY_GLOW_SQUID_AMBIENT, 1, 1);
        block
            .getLocation()
            .getWorld()
            .playSound(block.getLocation(), Sound.BLOCK_AMETHYST_BLOCK_CHIME, 2, 1);
        block
            .getLocation()
            .getWorld()
            .playSound(block.getLocation(), Sound.BLOCK_AZALEA_LEAVES_PLACE, 1, 0);
        block
            .getLocation()
            .getWorld()
            .playSound(block.getLocation(), Sound.BLOCK_SWEET_BERRY_BUSH_PLACE, 1, 1);
      ...
      case MINERALS -> {
        if (chance % 2 == 0) {
          block
              .getLocation()
              .getWorld()
              .playSound(block.getLocation(), Sound.BLOCK_GRINDSTONE_USE, 1, -1);
        } else {
          block
              .getLocation()
              .getWorld()
              .playSound(block.getLocation(), Sound.ENTITY_VILLAGER_WORK_MASON, 1, 0);
        }
        ...
        if (clusterChance < 5) {
          block.setType(
              getCluster(this.originalBlock) != null
                  ? getCluster(this.originalBlock)
                  : Material.RAW_IRON);
        } else {
