...
public class OilFieldTask extends BukkitRunnable {
  private final List<OilFieldCrate> crates;
  private final List<OilFieldTask> tasks = new ArrayList<>();
  private final FrontierLocation frontierLocation;
  ...
  public void spawnCrate(World world, FrontierLocation frontierLocation) {
    ...
    Location location =
        new Location(
            world, activeCrate.getCrateX(), activeCrate.getCrateY(), activeCrate.getCrateZ());
    ...
    for (int i = 0; i < cratesByLoc.size(); i++) {
      if (cratesByLoc.get(i).equals(this.activeCrate)) {
        ...
        Bukkit.getServer()
            .broadcastMessage("A crate has spawned at " + frontierLocation.getLocName());
      } else {
