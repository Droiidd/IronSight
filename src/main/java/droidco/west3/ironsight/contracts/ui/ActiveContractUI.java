...
public class ActiveContractUI {
  public static Inventory openActiveContractUi(Player p, Contract contract) {
    Inventory contractUi =
        Bukkit.createInventory(p, 54, ChatColor.DARK_GRAY + "Active Contract info:");
    ...
    for (CompletionStep step : steps) {
      ...
      ItemStack reqSlot =
          getStepRequestedItem(stepNum, step.getRequestedGoods(), contract.getRequestedAmount());
  ...
  public static ItemStack getStepLocationIcon(int stepNumber, String stepLoc) {
    ...
    List<String> lore = new ArrayList<>();
    lore.add(ChatColor.GRAY + stepLoc);
    meta.setLore(lore);
    item.setItemMeta(meta);
    return item;
  }
  ...
  public static ItemStack getStepRequestedItem(
      int stepNumber, ItemStack requestedItem, int requestAmount) {
    ItemStack item = null;
    if (requestedItem != null) {
      ...
      String requestMessage =
          ChatColor.WHITE
              ...
              + requestAmount
              + " "
              + requestedItem.getItemMeta().getDisplayName();
