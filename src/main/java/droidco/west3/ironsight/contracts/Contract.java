...
public class Contract {
  ...
  public void generateNewDelivery() {
    ...
    switch (deliveryType) {
      case FISHER -> {
        ...
        addCompletionStep(
            ...
            desc,
            requestedItem,
            "Ride to " + ChatColor.GREEN + frontierLocation.getLocName());
      }
      case MINER -> {
        List<String> desc = new ArrayList<>();
        desc.add("Arrive at " + ChatColor.GREEN + frontierLocation.getLocName());
    ...
    String listing = "";
    if (rareRequest) {
      listing += (ChatColor.LIGHT_PURPLE + "Rare ");
    }
    switch (deliveryType) {
      case FISHER -> {
        listing += (ChatColor.WHITE + "Fishing Guild ");
      }
      case MINER -> {
        listing += (ChatColor.WHITE + "Mining Guild ");
      }
      case DRUG_RUNNER -> {
        listing += (ChatColor.RED + "Drug Runner ");
      }
    }

    if (bulkOrder) {
      listing += (String.valueOf(ChatColor.WHITE) + ChatColor.BOLD + "Bulk ");
    }
    listing += (ChatColor.WHITE + "Order!");
