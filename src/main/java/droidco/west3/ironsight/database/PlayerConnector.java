...
public class PlayerConnector {
  ...
  public static void fetchPlayerHorses(Connection conn, Player p) {
    try {
      String sql = "select * from horse where bandit_id = '" + p.getUniqueId() + "'";
    ...
    for (FrontierHorse horse : horses) {
      try {
        String itemSql =
            "select * from custom_item where bandit_id = '"
                + p.getUniqueId()
                + "' AND "
                + "storage_type = '"
                + horse.getHorseName()
  ...
  public static boolean fetchPlayerData(Connection conn, Player p) {
    try {
      ...
      String sql = "select * from bandit where bandit.pId = '" + p.getUniqueId() + "'";
  ...
  public static void savePlayer(Bandit b, Connection conn) throws SQLException {
    ...
    if (updateVal > 0) {
      ...
      try {
        String sqlInsert =
            "insert into bandit (pId, wallet, bank, isBleeding, brokenLegs, isWanted, isJailed, isCombatBlocked, bounty, contractorXp, contractorLvl, wantedKills, contractor_title, jailStartTime, vault_size, vault_level) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
  ...
  public static void saveInventory(Player p, ItemStack[] items, String storageType, Connection conn)
      throws SQLException {
    //        delete
    //        from custom_item
    ...
    try {
      ...
      try {
        String sqlInsert =
            "insert into custom_item (bandit_id, storage_type, item_contents) values (?,?,?);";
        PreparedStatement insertStmt = conn.prepareStatement(sqlInsert);
        insertStmt.setString(1, p.getUniqueId().toString());
  ...
  public static void resetInventories(Player p, Connection conn) throws SQLException {
    try {
      String sqlInsert =
          "delete from custom_item where custom_item.bandit_id = '" + p.getUniqueId() + "'";
      PreparedStatement insertStmt = conn.prepareStatement(sqlInsert);
      int insertVal = insertStmt.executeUpdate();
