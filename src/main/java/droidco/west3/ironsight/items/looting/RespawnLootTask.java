...
public class RespawnLootTask extends BukkitRunnable {
  ...
  // PlayerCore core,
  public RespawnLootTask(
      IronSight plugin, int respawnTime, Location blockLoc, ItemStack[] prevContents) {
    this.plugin = plugin;
    tick = 0;
    seconds = 0;
    ...
    Location hologramLoc =
        new Location(
            blockLoc.getWorld(), blockLoc.getX() + 0.5, blockLoc.getY() - 1, blockLoc.getZ() + 0.5);
